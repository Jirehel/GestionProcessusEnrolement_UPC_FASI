<?php
    ...
    $idd=0; 
    include './controllers/all_listes_enrollement_controlleur.php';
    $badg_c=new all_listes_enrollement_controlleur();
    if (!isset($_GET['sessionid']) OR !is_numeric($_GET['sessionid']) OR empty($_GET['sessionid'])) {
        header("Location:index.php?p=all_sessions");
    }else{
        $idd=sanitize($_GET['sessionid']);
    }
...
<section class="content">
    <div class="container-fluid">
        <div class="block-header">
            <h2>TOUTES LES LISTE D'ENROLEMENT PAR PROMOTIONS</h2>
        </div>
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="list-group">
                <?php foreach ($badg_c->liste_enrollements($idd) as $entry) { ?>
                <a href="<?="index.php?p=enr_liste&liste=".$entry[0]."&session=".$idd?>" class="list-group-item list-group-item-action"><?="LISTE ENROLEMENT ". $badg_c->promotion($entry[2]) ?></a>
                <?php }?>
            </div>
            </div>
        </div>
